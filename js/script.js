...
var omega3 = 0,theta3 = 0, alpha3 = 0;
var vela = 0,
  velba = 0,
  velb = 0;
var accta = 0,
...
let clockbtn = `
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#089B93" class="icon">
<path d="M11.5 20.5C6.80558 20.5 3 16.6944 3 12C3 7.30558 6.80558 3.5 11.5 3.5C16.1944 3.5 20 7.30558 20 12C20 13.5433 19.5887 14.9905 18.8698 16.238M22.5 15L18.8698 16.238M17.1747 12.3832L18.5289 16.3542L18.8698 16.238"  stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
  <span>Rotate</span>
`;
...
`
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#089B93" class="icon" >
  <path d="M5.06697 5.91046L5.65392 1.99997"  stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  <path d="M5.06697 5.91046L8.80751 7.19296"  stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  <path d="M3 12.0609C3 17.0314 7.02944 21.0609 12 21.0609C16.9706 21.0609 21 17.0314 21 12.0609C21 7.09029 16.9706 3.06085 12 3.06085C9.92877 3.06085 8.02095 3.76052 6.5 4.9364"  stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  <span>Rotate</span>
</svg>
`

let playBtn = ` 
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#089B93" stroke="#ffffff" class="icon">
...
function startsim() {
  const simLoadText = document.querySelector('.sim-load');
  simLoadText.innerHTML="";
...
function simstate() {
  simstatus = !simstatus;
  const simPlayPauseBtn = document.querySelectorAll('.playPause-btn');
if (simstatus == 0) {
    clearInterval(simTimeId);
    // simstatus = 1;
    $("#theta2spinner").spinner("value", theta2); //to set simulation parameters on pause
    pauseTime = setInterval("varupdate();", "100");
    simPlayPauseBtn.forEach(btn => btn.innerHTML = pauseBtn);
  }
  if (simstatus == 1) {
    time = 0;
    // simstatus = 0;
    ...
    simTimeId = setInterval("time=time+0.1; varupdate(); ", "100");
    simPlayPauseBtn.forEach(btn => btn.innerHTML = playBtn);
  } 
...
function rotstate() {
  rotstatus = rotstatus == 1 ? -1 : 1;
  const rotateBtn = document.querySelectorAll('.rotate-btn');

  if (rotstatus == 1) {
    rotateBtn.forEach(btn => btn.innerHTML = anticlockbtn);
    // rotstatus = -1;
  }
  if (rotstatus == -1) {
    rotateBtn.forEach(btn => btn.innerHTML = clockbtn);
    // rotstatus = 1;
  }
...
function varupdate() {
  ...
  if (!simstatus) {
    ...
    commenttext = `<p>Acceleration analysis of Slider Crank Mechanism</p><p>V<sub>yx</sub>=Velocity of y with respect to x<br>A<sub>yx</sub>=Acceleration of y with respect to x</p>`;
    printcomment("<p>Acceleration analysis of Slider Crank Mechanism</p>");
  }
  if (simstatus) {
    ...
    commenttext = `<p>Acceleration Diagram (at &theta;<sub>2</sub> = ${theta2}&deg;)</p><p>V<sub>yx</sub>=Velocity of y with respect to x<br>A<sub>yx</sub>=Acceleration of y with respect to x</p>`;
    printcomment("<p>Acceleration Diagram (at &theta;<sub>2</sub> =" + theta2 + "&deg;)</p>");
  ...
  printcomment(
    "<p>V<sub>yx</sub>=Velocity of y with respect to x<br>A<sub>yx</sub>=Acceleration of y with respect to x</p>"
  );
  draw();
}
...
const resultsLocation  = document.querySelector(".task-section--results");
function printcomment() {
  resultsLocation.innerHTML = commenttext;
}


// scroll to the div location
function scrollToSimulation() {
  const simulationLocation = document.querySelector(".main");
...
function scrollToResults() {
  const resultsLocationLocation = document.querySelector(".result");
